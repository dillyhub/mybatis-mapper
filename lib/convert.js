...
var convertParametersInner = function(convertString, param) {
  var stringTarget = [];

  var regexes = ['#', '$'].map(prefix => { return new RegExp('(\\' + prefix + '\\{[a-zA-Z0-9._\\$]+\\})', 'g'); });
  for(var i = 0; i < regexes.length; i++) {
    var matches = convertString.match(regexes[i]);
    if(matches && matches.length > 0) {
      stringTarget = stringTarget.concat(matches);
    }
  }
  ...
  if (stringTarget != null && stringTarget.length > 0){
    ...
    for (var i=0; i<stringTarget.length; i++) {
      ...
      var prefix = target[0];
      var t = target.replace(prefix + '{', '').replace('}','');
      var tempParamKey = eval('param.' + t);

      if(tempParamKey === undefined) {
        throw new Error("Parameter " + target + " is not converted.");
      }

      var reg = new RegExp('\\' + prefix + '{' + t + '}', 'g');
